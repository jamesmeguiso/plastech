...
<body>
    ...
    <div class="vendo-card">
        <div class="header-banner">
            PLAS-TECH ECO-VENDO PORTAL
        </div>

        <div class="content-body">
            <div id="statusBanner" class="connection-status">
                🔴 Internet Locked (Insert Bottles)
            </div>

            <div class="network-info">
                System Online | Total Inserted: <strong id="mainBottleCount"><?php echo $bottleCount; ?></strong>
            </div>
            ...
            <!-- Vendo Stacked Buttons -->
            <button class="vende-btn btn-green" onclick="openInsertModal()">
                Insert Bottles Now
            </button>
            <button class="vende-btn btn-blue" onclick="openRatesModal()">
                View Rates
            </button>
            <button class="vende-btn btn-red" onclick="alert('System Hardware: IR Sensor & Cooling Fan Operational.')">
                Machine Status
            </button>
        </div>

        <!-- Talisay City College Footer with Admin Access -->
    ...
    <div id="insertModal" class="modal-overlay">
        <div class="modal-box">
            ...
            <div class="live-count-badge">
                Session Count: <span id="modalBottleCount">0</span> Bottles
            </div>
            ...
            <button class="vende-btn btn-red" style="margin-top: 10px; padding: 10px; margin-bottom: 0;" onclick="closeInsertModal()">
                Done / Finish
            </button>
        </div>
    </div>
    ...
    <div id="ratesModal" class="modal-overlay">
        <div class="modal-box">
            ...
            <p style="font-size: 12px; color: #666; margin-bottom: 5px;">Recycle plastic bottles to earn internet runtime:</p>
            ...
            <button class="vende-btn btn-blue" style="margin-top: 10px; padding: 10px; margin-bottom: 0;" onclick="closeRatesModal()">
                Close
            </button>
        </div>
    </div>
    ...
    <div id="adminLoginModal" class="modal-overlay">
        <div class="modal-box">
            ...
            <p style="font-size: 12px; color: #666;">Enter credentials to manage kiosk parameters.</p>
            ...
            <button class="vende-btn btn-dark" style="margin-top: 5px; padding: 10px;" onclick="verifyAdminLogin()">
                Login
            </button>
            <button class="vende-btn btn-red" style="padding: 10px; margin-bottom: 0;" onclick="closeAdminLogin()">
                Cancel
            </button>
        </div>
    </div>
    ...
    <div id="adminDashboardModal" class="modal-overlay">
        <div class="modal-box" style="max-width: 360px;">
            ...
            <div style="margin-bottom: 12px; text-align: left;">
                <label style="font-size: 11px; font-weight: bold; color: #555;">Manage Specific User IP:</label>
                <input type="text" id="targetIpInput" class="form-input" placeholder="e.g. 192.168.4.15" style="margin: 4px 0 6px 0; padding: 6px; font-size: 12px;">
                <button class="vende-btn btn-blue" style="padding: 8px; font-size: 12px; margin-bottom: 6px;" onclick="resetSpecificUser()">
                    Reset User's Time
                </button>
            </div>

            <button class="vende-btn btn-red" style="padding: 10px; font-size: 13px; margin-bottom: 8px;" onclick="resetAllUsersTime()">
                ⚡ Reset All Users Time
            </button>
            <button class="vende-btn btn-dark" style="padding: 8px; font-size: 12px; margin-bottom: 0;" onclick="closeAdminDashboard()">
                Logout / Close
            </button>
        </div>
    </div>
    ...
    function verifyAdminLogin() {
        ...
        if (u === 'group5' && p === 'snire') {
            ...
            alert('Invalid credentials! Use user: group5, pass: snire');
        }
    }
    ...
    function resetSpecificUser() {
        let ip = document.getElementById('targetIpInput').value.trim();
        if (!ip) {
            alert('Please enter a valid user IP address.');
            return;
        }
        fetch('authorize_net.php?ip=' + ip + '&auth=0')
            .then(res => res.json())
            .then(data => {
                alert('Time and connection successfully revoked for IP: ' + ip);
                document.getElementById('targetIpInput').value = '';
            }).catch(e => {
                alert('Error resetting user time.');
            });
    }

    function resetAllUsersTime() {
        if (confirm('Are you sure you want to wipe all session times and lock all active users?')) {
            sessionSeconds = 0;
            sessionEarnedBottles = 0;
            updateInternetAccess(false);

            // Calls backend reset script to clear status.json completely
            fetch('reset_time.php')
                .then(res => res.json())
                .then(data => {
                    alert('All users time have been successfully reset.');
                    closeAdminDashboard();
                    syncDatabaseStats();
                })
                .catch(err => {
                    alert('Error resetting time on the server.');
                });
        }
    }
        ...
        fetch('status.json?' + new Date().getTime())
            ...
            .then(data => {
                if (data && data.bottles !== undefined) {
                    ...
                    if (data.seconds !== undefined && data.seconds > sessionEarnedBottles) {
